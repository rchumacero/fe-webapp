...
type FormFormData = z.infer<typeof formValidationSchema>;
...
export default function FormFormPage({ id, processId }: FormFormProps) {
  ...
  useEffect(() => {
    ...
    if (processId && !id) {
      setValue('processId', processId, { shouldDirty: false });
    }
  }, [vendor, processId, id, setValue]);

  // Load existing form data if id is provided
  useEffect(() => {
    if (id) {
      const fetchForm = async () => {
        ...
        try {
          ...
          reset({
            ...
            moduleCode: form.moduleCode || 'workflow',
            processId: (form as any).processId || '',
  ...
  const onSubmit: SubmitHandler<FormFormData> = async (formData) => {
    setIsSubmitting(true);
    try {
      const payload = {
        ...formData,
        processId: formData.processId || processId || '',
      };
      if (id) {
      ...
      router.push(FORM_ROUTES.LIST(payload.processId));
      router.refresh();
    } catch (error) {
  ...
  const handleCancel = (e: React.MouseEvent) => {
    ...
    const currentProcessId = control._defaultValues.processId || processId;
    router.push(FORM_ROUTES.LIST(currentProcessId));
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-foreground">
              {id ? t(FORM_CONSTANTS.EDIT_TITLE) || 'Edit Form' : t(FORM_CONSTANTS.CREATE_TITLE) || 'Create Form'}
            </h1>
            <p className="text-muted-foreground mt-1 text-sm">
              {id
                ? t(FORM_CONSTANTS.DESCRIPTION_EDIT) || 'Modify properties and active status for this dynamic form model.'
                : t(FORM_CONSTANTS.DESCRIPTION_CREATE) || 'Define a new schema with module key, custom code identifier, and name.'}
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <input type="hidden" {...register("processId")} />
        <div className="lg:col-span-2">
          {errors.processId && (
            <div className="mb-4 p-3 bg-destructive/10 border border-destructive text-destructive rounded-md text-sm font-medium flex items-center gap-2">
              <AlertCircle className="h-4 w-4" />
              {errors.processId.message || "A valid Process ID is required to save this form."}
            </div>
          )}
          <Card className="border-border/40 shadow-xl overflow-hidden bg-card/50 backdrop-blur-sm h-full">
            <CardContent className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-1 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground ml-1">
                    {t(FORM_CONSTANTS.MODULE) || 'Module Code'}
                  </label>
                  <Input
                    {...register("moduleCode")}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder="Provide a concise description of the form's target attributes or workflow usage..."
                  rows={4}
          ...
          <Card className="border-primary/20 bg-primary/5 shadow-xl border-dashed">
            <CardHeader>
              <CardTitle className="text-base font-bold text-primary flex items-center gap-2">
                ...
                {t('common.formStatus') || 'Form Status'}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-2.5 text-xs text-muted-foreground leading-relaxed pt-2">
                ...
                <span>
                  {t('common.formWarning') || 'Please make sure all data schema elements are entered accurately before committing saves.'}
                </span>
              </div>
            </CardContent>
      ...
      <ConfirmDialog
        ...
        description={t('workflow.form.cancel_warning') || "You have unsaved form modifications. Are you sure you want to discard your edits and return to the list?"}
