...
import { Profile, ProfileRepositoryImpl } from '@kplian/infrastructure';
...
import { RefreshCw, Plus, Search, Trash2, Edit2, Loader2, Save, MoreHorizontal, UserSquare2, Terminal, Info, Globe, Building } from 'lucide-react';
...
export default function ProfileListPage() {
  ...
  const handleOpenEdit = (profile: Profile) => {
    ...
    setFormCode(profile.code);
    setFormName(profile.name);
    setFormModuleCode(profile.moduleCode);
    setFormVendorCode(profile.vendorCode || '');
    setDialogOpen(true);
  };
  ...
  const filteredProfiles = profiles.filter(profile => {
    ...
    return (
      ...
      (profile.moduleCode || '').toLowerCase().includes(term) ||
      (profile.vendorCode || '').toLowerCase().includes(term)
    );
  });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10 px-4 md:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex justify-between items-center bg-background/80 backdrop-blur-md sticky top-0 z-10 py-4 border-b border-border/10">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
            <UserSquare2 className="size-6 text-primary" /> User Profiles
          </h1>
          <p className="text-sm text-muted-foreground mt-1">Configure and manage authorization profiles for system access</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={fetchProfiles} className="rounded-full hover:bg-accent hover:rotate-180 transition-all duration-500">
      ...
      <div className="relative w-full max-w-md">
        ...
        <Input
          placeholder="Search profiles by code, name or module..."
      ...
      {loading ? (
        ...
        <div className="text-center py-20 border border-dashed border-border/50 rounded-3xl bg-card/10">
          <p className="text-muted-foreground text-sm">No user profiles registered.</p>
        </div>
      ) : (
              ...
              <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-36 bg-card border-border/40">
                    <DropdownMenuItem onClick={() => handleOpenEdit(profile)} className="cursor-pointer text-foreground text-xs gap-2">
                      <Edit2 className="size-3.5" /> Edit Profile
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => handleDelete(profile.id)} className="text-destructive cursor-pointer focus:bg-destructive/10 text-xs gap-2">
              ...
              <CardContent className="space-y-3 pt-0 pb-6">
                ...
                {profile.status && (
                  <div className="flex items-center gap-2 mt-2">
                    <Badge variant={profile.status === 'ACTIVE' ? 'default' : 'secondary'} className="text-[9px] px-2 py-0.5 rounded-full uppercase">
                      {profile.status}
                    </Badge>
                  </div>
                )}
        ...
        <DialogContent className="max-w-md bg-card border-border/40 text-foreground rounded-3xl p-6">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold flex items-center gap-2">
              ...
              {editingProfile ? 'Modify User Profile' : 'Register New Profile'}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSave} className="space-y-4 py-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Profile Code</label>
              <Input
                value={formCode}
