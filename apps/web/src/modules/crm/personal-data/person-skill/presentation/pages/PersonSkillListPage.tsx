...
export const PersonSkillListPage = ({ personId }: PersonSkillListPageProps) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await personSkillRepository.delete(deleteTargetId);
      fetchSkills();
    } catch (error) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {!isLoading && filteredSkills.length === 0 && (
          <div className="col-span-full py-12 text-center border-2 border-dashed border-border/40 rounded-xl bg-accent/5">
            ...
            <p className="text-muted-foreground font-medium">{t(PERSON_SKILL_CONSTANTS.RECORD_NOT_FOUND)}</p>
          </div>
        )}
      </div>
      <ConfirmDialog
        ...
        title={t(PERSON_SKILL_CONSTANTS.CONFIRM_DELETE)}
        description={t(PERSON_SKILL_CONSTANTS.FORM.DIRTY_WARNING) || "This action cannot be undone."}
        confirmText={t(PERSON_SKILL_CONSTANTS.FORM.SUBMIT)}
        cancelText={t(PERSON_SKILL_CONSTANTS.FORM.CANCEL)}
