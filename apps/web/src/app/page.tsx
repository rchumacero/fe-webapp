  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 bg-emerald-100 dark:bg-[#09261b] border-none p-10 flex flex-col justify-between relative overflow-hidden h-72">
          <div className="z-10 space-y-4">
            <h2 className="text-3xl font-bold text-emerald-900 dark:text-white transition-colors">
              {t('dashboard.congratulations')}<br /> Jaydon Frankie
            </h2>
            <p className="text-emerald-700 dark:text-emerald-300/80 max-w-md transition-colors">
              {t('dashboard.best_seller', { percentage: 57.6 })}
            </p>
        ...
        <Card className="bg-red-50 dark:bg-[#450a0a] border-none relative overflow-hidden group h-72">
          ...
          <div className="relative z-10 p-8 h-full flex flex-col justify-between">
            <div>
              <span className="text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-widest">{t('dashboard.new')}</span>
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-2 group-hover:scale-105 transition-transform">{t('dashboard.mountain_boots')}</h3>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: t('dashboard.product_sold'), value: "765", change: "+2.6%", up: true },
          { label: t('dashboard.total_balance'), value: "18,765", change: "-0.1%", up: false },
          { label: t('dashboard.sales_profit'), value: "4,876", change: "+0.6%", up: true }
        ].map((stat) => (
          <Card key={stat.label} className="bg-card dark:bg-[#161b22] border-none p-8 space-y-4 shadow-xl transition-colors">
            <div className="flex justify-between items-start">
              ...
              <div className="flex items-center gap-1 text-[13px] font-bold">
                ...
                <span className={stat.up ? "text-emerald-600" : "text-red-600"}>{stat.change} {t('dashboard.last_week')}</span>
              </div>
            </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-8">
          ...
          <Card className="bg-card dark:bg-[#161b22] border-none p-8 flex flex-col h-[450px] transition-colors">
            <h5 className="text-lg font-bold text-foreground mb-8 transition-colors">{t('dashboard.sale_by_gender')}</h5>
          ...
          <Card className="lg:col-span-2 bg-card dark:bg-[#161b22] border-none p-8 h-[450px] relative transition-colors">
            <div className="flex justify-between items-center mb-10">
              <div>
                <h5 className="text-lg font-bold text-foreground transition-colors">{t('dashboard.yearly_sales')}</h5>
                <p className="text-sm text-emerald-600 dark:text-emerald-500 font-bold mt-1 transition-colors">{t('dashboard.more_than_last_year', { percentage: 43 })}</p>
              </div>
