...
export function useInventory() {
  ...
  // Selected Inventory for detail view
  ...
  const saveInventory = useCallback(async () => {
    ...
    const payload = {
      ...
      inventoryDate: inventoryDraft.inventoryDate,
    ...
    console.log('[useInventory Debug] Saving Inventory Payload:', JSON.stringify(payload, null, 2));

    try {
      const created = await inventoryRepo.create(payload as any);
  ...
  }, [vendorCode, inventoryDraft, details, fetchInventories, resetForm]);

  // Filter inventories by warehouse or date
